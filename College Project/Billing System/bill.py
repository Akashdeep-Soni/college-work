...
class Bill_App:
    def __init__(self,root):
        ...
        Hairs_s_txt=Entry(F2,width=10,textvariable=self.spray,font=('times new roman',16,'bold'),bd=5,relief=SUNKEN).grid(row=3,column=1,padx=10,pady=10)
        ...
        total_btn=Button(btn_F,command=self.total,text="Total",bg='cadetblue',fg='white',bd=3,pady=15,width=10,font='arial 15 bold').grid(row=0,column=0,padx=5,pady=5)
        GBill_btn=Button(btn_F,command=self.bill_area,text="Genrate Bill",bg='cadetblue',fg='white',bd=3,pady=15,width=10,font='arial 15 bold').grid(row=0,column=1,padx=5,pady=5)
        Clear_btn=Button(btn_F,command=self.clear_data,text="Clear",bg='cadetblue',fg='white',bd=3,pady=15,width=10,font='arial 15 bold').grid(row=0,column=2,padx=5,pady=5)
        Exit_btn=Button(btn_F,command=self.exit_app,text="Exit",bg='cadetblue',fg='white',bd=3,pady=15,width=10,font='arial 15 bold').grid(row=0,column=3,padx=5,pady=5)
        self.welcome_bill()
    ...
    def bill_area(self):
        if self.c_name.get()=="" or self.c_phon.get()=="":
            ...
            if self.loshan.get()!=0:
                self.txtarea.insert(END,f"\nBody Lotion\t\t{self.loshan.get()}\t\t{self.loshan.get()*180}")
            ...
            if self.tea.get()!=0:
                self.txtarea.insert(END,f"\nTea\t\t{self.tea.get()}\t\t{self.tea.get()*150}")
                
            #========Cold Dringk
            if self.maza.get()!=0:
                self.txtarea.insert(END,f"\nMaza\t\t{self.maza.get()}\t\t{self.maza.get()*60}")
            if self.cock.get()!=0:
                self.txtarea.insert(END,f"\nCock\t\t{self.cock.get()}\t\t{self.cock.get()*60}")
            if self.frooti.get()!=0:
                self.txtarea.insert(END,f"\nFrooti\t\t{self.frooti.get()}\t\t{self.frooti.get()*50}")
            if self.thumbsup.get()!=0:
                self.txtarea.insert(END,f"\nThumbs Up\t\t{self.thumbsup.get()}\t\t{self.thumbsup.get()*45}")
            if self.limca.get()!=0:
                self.txtarea.insert(END,f"\nLimca\t\t{self.limca.get()}\t\t{self.limca.get()*40}")
            if self.sprite.get()!=0:
                self.txtarea.insert(END,f"\nSprite\t\t{self.sprite.get()}\t\t{self.sprite.get()*60}")
    ...
    def clear_data(self):
        op=messagebox.askyesno("Clear", "Do you really clear the data?")
        if op>0:
            #=========variable
            #=========Cosmetic
            self.soap.set(0)
